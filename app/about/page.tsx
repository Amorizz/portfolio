  ...
  return (
    <main className="min-h-screen py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-12 sm:space-y-16 lg:space-y-24">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 items-center justify-center">
          {/* Profile Image - Left */}
        ...
        {/* Who I Am */}
        <section className="px-0 sm:px-8 lg:px-20 xl:px-40">
        ...
        <section className="relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-accent/5 via-transparent to-accent/5 rounded-2xl sm:rounded-3xl"></div>
          ...
          <div className="relative max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              {/* CV Card */}
              <div className="bg-card border border-border rounded-xl sm:rounded-2xl p-6 sm:p-8 hover:border-accent/50 transition-all group">
              ...
              <div className="bg-card border border-border rounded-xl sm:rounded-2xl p-6 sm:p-8 hover:border-accent/50 transition-all group">
                <div className="flex flex-col items-center text-center space-y-4">
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-accent/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                  ...
                  <div className="space-y-2 sm:space-y-3 w-full text-left">
                    <a 
                      href={`mailto:${personalInfo.email}`}
                      className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-muted-foreground hover:text-accent transition-colors"
                    >
                      <Mail className="h-3.5 w-3.5 sm:h-4 sm:w-4 flex-shrink-0" />
                      <span className="truncate text-xs sm:text-sm">{personalInfo.email}</span>
                    </a>
                    <a 
                      href={`tel:${personalInfo.phone}`}
                      className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-muted-foreground hover:text-accent transition-colors"
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {skills.map((skill) => (
              <div key={skill.id} className="bg-card/30 border border-border rounded-lg sm:rounded-xl p-4 sm:p-6 hover:border-accent/50 transition-all">
                <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">{skill.icon}</div>
                <h3 className="text-sm sm:text-base font-semibold text-foreground mb-1.5 sm:mb-2">{skill.title}</h3>
                <p className="text-xs text-muted-foreground mb-2 sm:mb-3">{skill.description}</p>
                <div className="flex flex-wrap gap-1 sm:gap-1.5">
                  {skill.skills.map((item, index) => (
                    <Badge key={index} className="bg-violet-600/10 text-foreground hover:bg-violet-600/20 text-[9px] sm:text-[10px] px-1.5 py-0.5">
                      {item}
                    </Badge>
                  ))}
                </div>
